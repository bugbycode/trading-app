...
//@EnableScheduling
@Deprecated
public class FuturesTaskWeek {

	private final Logger logger = LogManager.getLogger(FuturesTaskWeek.class);
	...
	public void continuousKlines() throws Exception {
		try {
			...
			for(String pair : pairArr) {
				...
				List<Klines> klinesList_lastweek_1h = klinesService.continuousKlines_lastweek_1h(pair, restBaseUrl);
				Klines highKline_lastweek_1h = PriceUtil.getMaxPriceKLine(klinesList_lastweek_1h);//本周期最高价
				Klines lowKline_lastweek_1h = PriceUtil.getMinPriceKLine(klinesList_lastweek_1h);//本周期最低价
				decimalNum = highKline_lastweek_1h.getDecimalNum();
				/*
				...
				*/
				...
				Klines lastKlines = klinesList_3h.get(rise_fall_offset);//前一小时K线
				...
				//1天为1个周期斐波那契回撤水平
				FibInfo fibInfo = new FibInfo(lowKline_lastweek_1h, highKline_lastweek_1h,decimalNum);
				...
				if(fib0 < fib1) {
					...
					} else if(closePrice < fib0 && openPrice >= fib0) {
						subject = pair + "永续合约跌破" + fib0 + " " + DateFormatUtil.format(new Date());
						text = "当前价格：" + closePrice;
					} else if(closePrice > fib1 && openPrice <= fib1) {
						subject = pair + "永续合约突破" + fib1 + " " + DateFormatUtil.format(new Date());
						text = "当前价格：" + closePrice;
					}
					...
					if(closePrice > fib786 && closePrice < fib66 && lowPrice < fib786) { //0.786~0.66
						...
						subject = pair + "永续合约突破" + fib0 + " " + DateFormatUtil.format(new Date());
						text = "当前价格：" + closePrice;
					} else if(closePrice < fib1 && openPrice >= fib1) {
						subject = pair + "永续合约跌破" + fib1 + " " + DateFormatUtil.format(new Date());
						text = "当前价格：" + closePrice;
					}
				}
