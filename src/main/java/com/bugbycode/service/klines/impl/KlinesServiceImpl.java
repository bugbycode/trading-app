...
public class KlinesServiceImpl implements KlinesService {
			...
			if(ps == PositionSide.LONG) {//做多
					...
					if(autoTradeType == AutoTradeType.DEFAULT) {//自定义止盈止损
						...
						activationPriceValue = new BigDecimal(
								PriceUtil.formatDoubleDecimal(
										PriceUtil.calculateLongActivationPrice_v2(priceInfo.getPriceDoubleValue(), activationPriceRatio, profitPrice),
										decimalNum
								)
							);
				...
				try {
					...
					if(autoTradeType == AutoTradeType.DEFAULT) {//自定义止盈止损
						...
						activationPriceValue = new BigDecimal(
								PriceUtil.formatDoubleDecimal(
										PriceUtil.calculateShortActivationPrice_v2(priceInfo.getPriceDoubleValue(), activationPriceRatio, profitPrice),
										decimalNum
								)
							);
	...
	public void futuresFibMonitor(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h,  List<Klines> list_15m) {
		...
		FibInfoFactory[] factorys = {
				new FibInfoFactoryImpl(list_15m, list_15m, PositionSide.LONG),
				new FibInfoFactoryImpl(list_15m, list_15m, PositionSide.SHORT),
		};
	...
	public void futuresPriceAction(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h,  List<Klines> list_15m) {
        ...
        factoryList.add(new PriceActionFactoryImpl(list_4h, list_1h, list_15m));
	...
	public void eoptionMonitor(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h,  List<Klines> list_15m) {
		...
		EoptionFactory[] factories = {
				new EoptionFactoryImpl(list_4h, list_1h, list_15m),
	...
	public void consolidationAreaMonitor(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h,  List<Klines> list_15m) {
		...
		AreaFactory[] factories = {
					new AreaFactoryImpl(list_1h, list_15m)
				};
			...
			for (int index = 0; index < openPrices.size(); index++) {
				...
				if(factory.isLong() && PriceUtil.isBreachLong(last, price.getPrice()) 
					...
					for(User u : userList) {
						...
						//开仓订阅提醒
						String subject = String.format("%s永续合约%s(%s)做多机会(PNL:%s%%) %s", pair, price.getCode().getDescription(),
								PriceUtil.formatDoubleDecimal(price.getPrice(), last.getDecimalNum()),
								PriceUtil.formatDoubleDecimal(profitPercent, 2),
								DateFormatUtil.format(new Date()));
					...
					for(User u : userList) {
						...
						//开仓订阅提醒
						String subject = String.format("%s永续合约%s(%s)做空机会(PNL:%s%%) %s", pair, price.getCode().getDescription(),
								PriceUtil.formatDoubleDecimal(price.getPrice(), last.getDecimalNum()),
								PriceUtil.formatDoubleDecimal(profitPercent, 2),
								DateFormatUtil.format(new Date()));
