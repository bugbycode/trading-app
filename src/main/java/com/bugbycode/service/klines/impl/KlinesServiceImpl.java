...
import com.bugbycode.factory.fibInfo.impl.FibInfoFactoryImpl_v6;
...
public class KlinesServiceImpl implements KlinesService {
	...
	public void futuresFibMonitor(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h,  List<Klines> list_15m) {
		...
		FibInfoFactory factory = new FibInfoFactoryImpl_v6(list_1h, list_1h, list_15m);
	...
	public void openLong_priceAction(List<OpenPrice> openPrices, FibInfo fibInfo, Klines afterLowKlines,
		...
		for(int index = 0;index < openPrices.size(); index++) {
			...
			if(PriceUtil.isLong_v2(price, klinesList_hit)
				...
				//市价做多
				this.tradingTaskPool.add(new TradingTask(this, pair, PositionSide.LONG, 0, 0, offset, fibInfo, AutoTradeType.PRICE_ACTION, fibInfo.getDecimalPoint()));
	...
	public void openShort_priceAction(List<OpenPrice> openPrices, FibInfo fibInfo, Klines afterHighKlines,
		...
		for(int index = 0;index < openPrices.size(); index++) {
			...
			if(PriceUtil.isShort_v2(price, klinesList_hit)
				...
				//市价做空
				this.tradingTaskPool.add(new TradingTask(this, pair, PositionSide.SHORT, 0, 0, offset,  fibInfo, AutoTradeType.PRICE_ACTION, fibInfo.getDecimalPoint()));
	...
	public void volumeMonitor(List<Klines> list_1d, List<Klines> list_4h, List<Klines> list_1h, List<Klines> list_15m) {
		...
		//if(last.getEma25() > last.getEma99() && PriceUtil.isLong_v2(last.getEma25(), list_15m)) {
		if(PriceUtil.verifyPowerful_v19(current, parent))	{
			
			subject = String.format("%s永续合约开始上涨 %s", pair, dateStr);
			
		//} else if(last.getEma25() < last.getEma99() && PriceUtil.isShort_v2(last.getEma25(), list_15m)) {
		} else if(PriceUtil.verifyDecliningPrice_v19(current, parent)) {	
