...
public class PriceActionFactoryImpl_v2 implements PriceActionFactory{
	...
	private void init() {
		...
		for(int index = list.size() - 1; index > 2;index--) {
			...
			if((mode == QuotationMode.LONG && PriceUtil.verifyDecliningPrice_v29(current, parent)) 
					|| (mode == QuotationMode.SHORT && PriceUtil.verifyPowerful_v29(current, parent))) {
				fibEnd = current;
	...
	private boolean verifyHigh(Klines current) {
		return current.getEma7() > current.getEma25() && current.getEma7() > 0 && current.getMacd() > 0;
	}
