...
public class FibInfoFactoryImpl_v4 implements FibInfoFactory {
	...
	private void init() {
		...
		for(int index = list.size() - 1; index > 0; index--) {
			...
			if((mode == QuotationMode.LONG && PriceUtil.isBullishSwallowing(current, parent)) 
					|| (mode == QuotationMode.SHORT && PriceUtil.isPutInto(current, parent))) {
				if(fibEnd == null) {
					...
					List<Klines> points = PriceUtil.subList(parent, fibEnd, list);
					MarketSentiment ms = new MarketSentiment(points);
					if(mode == QuotationMode.LONG) {
						//addPrices(new OpenPriceDetails(fibInfo.getFibCode(parent.getBodyHighPriceDoubleValue()), parent.getBodyHighPriceDoubleValue()));
						addPrices(new OpenPriceDetails(fibInfo.getFibCode(ms.getMinBodyLowPrice()), ms.getMinBodyLowPrice()));
