...
public class FibInfoFactoryImpl_v5 implements FibInfoFactory {
	...
	private void init() {
		...
		for(int index = list.size() - 1; index > 0; index--) {
			...
			if((mode == QuotationMode.LONG && PriceUtil.verifyPowerful_v21(current, parent)) 
					|| (mode == QuotationMode.SHORT && PriceUtil.verifyDecliningPrice_v21(current, parent))) {
				List<Klines> points = PriceUtil.subList(fibAfterFlag, current, list);
