		...
		for(int index = list.size() - 1; index > 1; index--) {
			...
			if(mode == QuotationMode.LONG) {
				if(PriceUtil.verifyPowerful_v10(current, parent, next)) {
					...
					addPrices(new OpenPriceDetails(fibInfo.getFibCode(parent.getBodyHighPriceDoubleValue()), parent.getBodyHighPriceDoubleValue()));
					
				} else if(PriceUtil.verifyDecliningPrice_v10(current, parent, next) || PriceUtil.verifyPowerful_v8(current, parent)) {
				...
				if(PriceUtil.verifyDecliningPrice_v10(current, parent, next)) {
					...
					addPrices(new OpenPriceDetails(fibInfo.getFibCode(parent.getBodyLowPriceDoubleValue()), parent.getBodyLowPriceDoubleValue()));
					
				} else if(PriceUtil.verifyPowerful_v10(current, parent, next) || PriceUtil.verifyDecliningPrice_v8(current, parent)) {
