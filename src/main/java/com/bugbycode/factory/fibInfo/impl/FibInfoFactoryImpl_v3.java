...
public class FibInfoFactoryImpl_v3 implements FibInfoFactory {
	...
	private void init() {
		if(CollectionUtils.isEmpty(list) || list.size() < 50 || list_trend.size() < 1 || CollectionUtils.isEmpty(list_15m)) {
			return;
		}
		...
		Klines last_day = PriceUtil.getLastKlines(list_trend);
		Klines last_day_after = PriceUtil.getAfterKlines(last_day, list_15m);
		if(last_day_after != null) {
			this.fibAfterKlines.addAll(PriceUtil.subList(last_day_after, list_15m));
		}
		...
		double openPrice = last_day.getClosePriceDoubleValue();
		FibCode openCode = fibInfo.getFibCode(openPrice);
		addPrices(new OpenPriceDetails(openCode, openPrice));
	}
	...
	private PositionSide getPositionSide() {
		...
		if(!CollectionUtils.isEmpty(this.fibAfterKlines)) {
			Klines last_15m = PriceUtil.getLastKlines(this.fibAfterKlines);
			if(verifyShort(last_day, last_15m)) {
				ps = PositionSide.SHORT;
			} else if(verifyLong(last_day, last_15m)) {
	...
	private boolean verifyLong(Klines last_day, Klines last_15m) {
		return last_day.getClosePriceDoubleValue() < last_15m.getClosePriceDoubleValue();
	}
	
	private boolean verifyShort(Klines last_day, Klines last_15m) {
		return last_day.getClosePriceDoubleValue() > last_15m.getClosePriceDoubleValue();
	}
