...
public class FibInfoFactoryImpl_v3 implements FibInfoFactory {
	 ...
	 * @param ps_mode LONG/SHORT
	 */
	public FibInfoFactoryImpl_v3(List<Klines> list, List<Klines> list_15m, PositionSide ps_mode) {
	...
	private void init(PositionSide ps) {
		...
		double fibValue = fibInfo.getFibValue(openCode);//开仓点（盘整区高低点）
		for(int index = list.size() - 1; index > 0; index--) {
			Klines current = list.get(index);
			if(current.lte(end)) {
				break;
