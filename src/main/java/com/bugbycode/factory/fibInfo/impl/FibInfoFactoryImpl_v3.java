	...
	private void init() {
		...
		for(int index = list.size() - 1; index > 0; index--) {
			...
			if(mode == QuotationMode.LONG && PriceUtil.isBreachLong(current, fibValue)) {
				addPrices(new OpenPriceDetails(openCode, current.getClosePriceDoubleValue(), current.getLowPriceDoubleValue()));
				break;
			} else if(mode == QuotationMode.SHORT && PriceUtil.isBreachShort(current, fibValue)) {
				addPrices(new OpenPriceDetails(openCode, current.getClosePriceDoubleValue(), current.getHighPriceDoubleValue()));
				break;
			}
