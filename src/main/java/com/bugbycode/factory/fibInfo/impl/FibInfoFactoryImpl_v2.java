...
public class FibInfoFactoryImpl_v2 implements FibInfoFactory {
	...
	private void init() {
		...
		if(ps == PositionSide.LONG) {
			...
			secondSubList = PriceUtil.subList(startAfterFlag, firstSubList);
		...
		FibCode hitCode = parentFibInfo.getFibCode(fibInfo.getFibValue(FibCode.FIB0));
		List<FibCode> openCodes = parentFibInfo.getOpenCode(hitCode);
		for(FibCode code : openCodes) {
			double openPrice = parentFibInfo.getFibValue(code);
