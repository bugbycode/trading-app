	...
	private void init() {
		...
		for(int index = fibAfterKlines.size() - 1; index > 0; index--) {
			...
			if((mode == QuotationMode.LONG && PriceUtil.verifyPowerful_v8(current, parent)) 
				...
				points.addAll(PriceUtil.subList(fibAfterFlag, current, fibAfterKlines));
				break;
			}
		}
		...
		if(!ms.isEmpty()) {
			...
			Klines high = null;
			Klines lowBody = null;
			Klines highBody = null;
			if(mode == QuotationMode.LONG) {
				low = ms.getLow();
				lowBody = ms.getLowBody();
				addPrices(new OpenPriceDetails(this.fibInfo.getFibCode(low.getLowPriceDoubleValue()), low.getLowPriceDoubleValue()));
				addPrices(new OpenPriceDetails(this.fibInfo.getFibCode(lowBody.getBodyLowPriceDoubleValue()), lowBody.getBodyLowPriceDoubleValue()));
			} else {
				high = ms.getHigh();
				highBody = ms.getHighBody();
				addPrices(new OpenPriceDetails(this.fibInfo.getFibCode(high.getHighPriceDoubleValue()), high.getHighPriceDoubleValue()));
				addPrices(new OpenPriceDetails(this.fibInfo.getFibCode(highBody.getBodyHighPriceDoubleValue()), highBody.getBodyHighPriceDoubleValue()));
			}
		}
		...
		fibAfterFlag = PriceUtil.getAfterKlines(end, this.list_15m);
