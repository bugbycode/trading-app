...
public class FibInfoFactoryImpl_v2 implements FibInfoFactory {
	...
	private void init() {
		...
		FibInfoFactoryImpl factory = new FibInfoFactoryImpl(list_15m, list, list_15m);
		...
		for(OpenPrice p : openPriceList) {
			if(p.getCode().gte(FibCode.FIB618)) {
		...
		Klines fibAfterFlag = PriceUtil.getAfterKlines(factory.getEnd(), this.list_15m);
