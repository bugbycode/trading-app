	...
	private void init() {
		...
		for(int index = fibSubList.size() - 1; index > 1; index--) {
			...
			if(mode == QuotationMode.LONG) {
				if(PriceUtil.verifyPowerful_v10(current, parent, next) && openPrices.isEmpty()) {
					...
					addPrices(openPrice);
				} else if(PriceUtil.verifyDecliningPrice_v10(current, parent, next) && !openPrices.isEmpty()) {
				...
				if(PriceUtil.verifyDecliningPrice_v10(current, parent, next) && openPrices.isEmpty()) {
					double openPriceValue = parent.getBodyLowPriceDoubleValue();
					OpenPrice openPrice = new OpenPriceDetails(fibInfo.getFibCode(openPriceValue), openPriceValue);
					addPrices(openPrice);
				} else if(PriceUtil.verifyPowerful_v10(current, parent, next) && !openPrices.isEmpty()) {
					List<Klines> sub_points = PriceUtil.subList(current, fibSubList);
					MarketSentiment ms = new MarketSentiment(sub_points);
