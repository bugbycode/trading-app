	...
	private void init() {
		...
		for(int index = list_hit.size() - 1; index > 0; index--) {
			...
			if((hitCode = getIsBreachFibCode(fibInfo, QuotationMode.LONG, current)) != null) {
				...
				double stopLoss = stopLossFibInfo.getFibValue(FibCode.FIB1_272);
				
				addPrices(new OpenPriceDetails(hitCode, current.getClosePriceDoubleValue(), stopLoss, firstTakeProfit, secondTakeProfit, AutoTradeType.AREA_INDEX, fibInfo));
				break;
