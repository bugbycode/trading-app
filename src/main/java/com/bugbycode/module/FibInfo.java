...
public class FibInfo {
	...
	public FibCode getTakeProfit_v2(FibCode code) {
		...
		} else if(code == FibCode.FIB618) { // 0.618 -> 0.236
	...
	public FibCode getPriceActionTakeProfit_v1(FibCode code) {
		...
		if(code == FibCode.FIB0) { // 0 - 0.5
			...
			takeProfit = FibCode.FIB786;
		} else if(code == FibCode.FIB618 || code == FibCode.FIB786) { // 0.618 || 0.786 - 1
			takeProfit = FibCode.FIB1;
		} else if(code == FibCode.FIB1) {// 1 - 1.618
			takeProfit = FibCode.FIB1_618;
		} else if(code == FibCode.FIB1_272) {// 1.272 - 2
			takeProfit = FibCode.FIB2;
	...
	public FibCode getPriceActionTakeProfit_nextCode(FibCode code) {
		...
		if(code == FibCode.FIB0) {// 0 - 0.236
			...
			takeProfit = FibCode.FIB618;
		} else if(code == FibCode.FIB618) { // 0.618 - 0.786
			takeProfit = FibCode.FIB786;
		} else if(code == FibCode.FIB786) {// 0.786 - 1
			takeProfit = FibCode.FIB1;
		} else if(code == FibCode.FIB1) {// 1 - 1.272
			takeProfit = FibCode.FIB1_272;
		} else if(code == FibCode.FIB1_272) {// 1.272 - 1.618
			takeProfit = FibCode.FIB1_618;
	 ...
	 * @param decimalPoint
	 */
	public FibInfo(double fib1,double fib0,int decimalPoint) {
		this.startPrice = fib1;
		this.endPrice = fib0;
		this.decimalPoint = decimalPoint;
		this.level = FibLevel.LEVEL_0;
	}
    ...
    public boolean isLong() {
    	return getQuotationMode() == QuotationMode.LONG;
    }
    
    public boolean isShort() {
    	return getQuotationMode() == QuotationMode.SHORT;
    }
	...
	public FibCode getNextFibCode(FibCode current) {
		...
		if(current == FibCode.FIB4_618) {
			...
			result = FibCode.FIB5;
		} else if(current == FibCode.FIB618) {
			result = FibCode.FIB382;
	...
	public FibCode getFibCode(double price) {
		...
		FibCode[] codes = FibCode.values();
		for(int index = 0; index < codes.length; index++) {
			FibCode c = codes[index];
			double c_value = getFibValue(c);
			if((isLong() && price <= c_value)
					|| (isShort() && price >= c_value)) {
				code = c;
				break;
			}
		}
	 ...
	 * @return
	 */
	public FibCode getFibCode_v2(double price) {
		
		FibCode code = FibCode.FIB236;
		FibCode[] codes = FibCode.values();
		for(int index = 0; index < codes.length; index++) {
			FibCode c = codes[index];
			if(c == FibCode.FIB0) {
